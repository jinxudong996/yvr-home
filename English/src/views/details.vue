<template>
	<div>
		...
		<div class="w1455 f26 newsFy">
			<div @click="getpreviousNews" v-if="!prevShow">Previous：<a href="javascript:void(0);">{{ previousNews.title }}</a>
			</div>
			<div style="opacity: .6;" v-show="prevShow" v-else>Previous：No more</div>
			<div @click="getnextNews" v-if="!nextShow">Next：<a href="javascript:void(0);">{{ nextNews.title }}</a></div>
			<div style="opacity: .6;" v-show="nextShow" v-else>Next：No more</div>
		</div>
	...
	export default {
		...
		methods: {
			...
			async getpreviousNews() {
				if (this.previousNews.title != "No more") {
					this.$router.push({
						path: '/details',
						query: {
			...
			async getnextNews() {
				if (this.nextNews.title != "No more") {
					this.$router.push({
						path: '/details',
						query: {
							id: this.nextNews.id,
